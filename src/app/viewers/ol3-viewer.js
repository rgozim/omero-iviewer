...
export default class Ol3Viewer extends EventSubscriber {
  /**
    ...
    changeImageSettings(params = {}) {
      ...
      } else if (typeof params.interpolate === 'boolean') {this.viewer.enableSmoothing(params.interpolate);}
    ...
    resizeViewer(params = {}) {
      ...
      if (typeof params.window_resize === 'boolean' && params.window_resize) {Ui.adjustSideBarsOnWindowResize();}
    ...
    getRegionsPropertyChange(params = {}) {
      ...
      for (let i in params.shapes) {
        ...
        if (refOrCopy && prop === 'deleted') {refOrCopy = Object.assign({}, refOrCopy);}
        if (typeof params.callback === 'function') {params.callback(refOrCopy, hasBeenModified);}
    ...
    setRegionsProperty(params = {}) {
      ...
      if (prop === 'visible') {this.viewer.setRegionsVisibility(params.value, params.shapes);} else if (prop === 'selected') {this.changeShapeSelection(params);} else if (prop === 'state') {this.deleteShapes(params);}
    }
    ...
    generateShapes(params = {}) {
      ...
      if (typeof params.roi_id !== 'number') {params.roi_id = this.image_config.regions_info.getNewRegionsId();}
      let theDims =
      ...
      params.shapes.map((def) => {
        try {
          if (!def.deleted) {
            ...
            if (typeof params.add_history === 'boolean') {opts.add_history = params.add_history;}
            if (typeof params.hist_id === 'number') {opts.hist_id = params.hist_id;}
    ...
    deleteShapes(params = {}) {
      ...
      if (params.value === 'delete') {this.viewer.deleteShapes(params.shapes, false, params.callback);} else if (params.value === 'undo') {this.viewer.deleteShapes(params.shapes, true);}
    }
      ...
      let updateMeasurements = () => {
        ...
        for (let i = 0; i < measurements.length; i++) {
          ...
          if (typeof measurement !== 'object' ||
                    measurement === null || typeof measurement.id !== 'string') {continue;}
          let shape =
    ...
    drawShape(params = {}) {
      ...
      if (unattached.indexOf('z') === -1 &&
            this.image_config.image_info.projection === PROJECTION.INTMAX) {unattached.push('z');}
    ...
    afterShapeStorage(params = {}) {
      ...
      for (let id in params.shapes) {
        ...
        if (shape) {
          ...
          if (wasNew) {
            ...
            if (selShapesIdx !== -1) {selShapes[selShapesIdx] = newShape.shape_id;}
          }
          ...
          if (shape.deleted) {
            ...
            if (typeof oldRoi !== 'undefined' &&
                        oldRoi.shapes instanceof Map) {oldRoi.shapes.delete(oldRoiAndShapeId.shape_id);}
      ...
      if (Misc.isArray(params.errors)) {
        ...
        for (let e in params.errors) {console.error(params.errors[e]);}
    ...
    playDimension(params = {}) {
      ...
      // the stop function
      let stopPlay = () => {
        if (this.player_info.handle !== null) {clearInterval(this.player_info.handle);}
      ...
      let dim = params.dim;
      let min_dim = 0;
      let max_dim = dims['max_' + dim] - 1;
      let pres_dim = dims[dim];
            ...
            setInterval(
              () => {
                try {
                  ...
                  if (!this.viewer.watchRenderStatus(true)) {this.viewer.getRenderStatus(true);}
    ...
    captureViewport(params = {}) {
      ...
      if (allConfigs) {params.zip_entry = this.image_config.image_info.image_name;}
